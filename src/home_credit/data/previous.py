...
def aggregate_previous_application() -> pd.DataFrame:
    ...
    result = agg_all.merge(agg_approved, on="SK_ID_CURR", how="left").merge(
        agg_refused, on="SK_ID_CURR", how="left"
    )
    result["PREV_APPROVAL_RATE"] = _safe_div(
        result["PREV_APPROVED_COUNT"].fillna(0),
        result["PREV_APP_COUNT"],
    )
    result["PREV_REFUSAL_RATE"] = _safe_div(
        result["PREV_REFUSED_COUNT"].fillna(0),
        result["PREV_APP_COUNT"],
    )
